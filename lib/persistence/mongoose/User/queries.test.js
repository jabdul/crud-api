...
const db = (0, _index.mongooseConnect)(_index.config);
...
describe('Users queries', () => {
  ...
  describe('findById', () => {
    ...
    it('cannot find a user with a uuid not in db', async () => {
      try {
        const user = await _factoryGirl.factory.create('User');
        await (async () => userQueries.findById({
          uuid: user.uuid
        })).rejects.toThrow();
      } catch ({
        error,
  ...
  describe('updateById', () => {
    it('update a single user', async () => {
      ...
      expect(updateUser).toHaveProperty('uuid'); // expect(user['uuid']).toEqual(payload['uuid'])
