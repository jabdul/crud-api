...
exports.default = async ({ dbConnect, schema, config, routes, services, plugins = [], postRegisterHook, swaggerOptions = {}, loggerOptions = {}, serverOptions = {}, dockerized, intializers, }) => {
    ...
    const app = new hapi_1.Server(Object.assign({ host: dockerized ? config.get('dockerizedHostname') : config.get('server.hostname'), port: config.get('server.port'), tls, debug: {
    ...
    await app.register([
        ...[
            ...
            { plugin: hapi_swagger_1.default, options: swaggerOptions },
            {
                ...
                options: Object.assign({ prettyPrint: process.env.NODE_ENV !== 'production', redact: {
                        paths: ['req.headers.authorization', '*.password'],
                        remove: true,
                    }, logPayload: true, ignorePaths: ['/monitoring/healthz'] }, loggerOptions),
            },
